...
📤 自动发布
 │
 └── 生成 Markdown → git commit → 存入 daily/ 目录
```
...
| Secret 名称 | 值 | 说明 |
|-------------|-----|------|
| `API_KEY` | 你的 API Key | 默认使用 [DeepSeek](https://platform.deepseek.com/)（极便宜，约 ¥0.01/天） |

> 💡 也支持 OpenAI 或任何兼容 API，只需额外设置环境变量 `API_BASE_URL` 和 `API_MODEL`。
